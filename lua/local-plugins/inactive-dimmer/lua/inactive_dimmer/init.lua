...
local enabled = false
local defaults = { dim_factor = 0.65 }
...
local function rebuild_namespace()
	...
	for _, name in ipairs(groups) do
		...
		if ok and hl and next(hl) ~= nil then
			...
			vim.api.nvim_get_hl(namespace, name, dim)
		end
	end
end
...
function M.setup(opts)
	...
	if augroup_id then
		pcall(vim.api.nvim_dev_augroup_by_id, augroup_id)
	end
	...
	user_cmd("InactiveDimEnable", M.enable, { desc = "Enable Inactive Window Dimming", })
	user_cmd("InactiveDimDisable", M.disable, { desc = "Enable Inactive Window Dimming", })
	user_cmd("InactiveDimToggle", M.toggle, { desc = "Enable Inactive Window Dimming", })
